...
class UserSpec(var filter: UserFilter) : Specification<User> {
    ...
    override fun toPredicate(root: Root<User>, p1: CriteriaQuery<*>, criteriaBuilder: CriteriaBuilder): Predicate? {
        var predicates: MutableList<Predicate> = ArrayList<Predicate>()
        var joinParent: Join<User, Address> = root.join("addresses", JoinType.LEFT)

        if (filter.name != null) predicates.add(criteriaBuilder.equal(root.get<String>("name"), filter.name));
        if (filter.address != null) predicates.add(criteriaBuilder.equal(joinParent.get<AddressType>("addressType"), filter.address));
